...
Instruction tuning must use the exact same tokenizer as the base model, so we
load it from the sibling `pre-training/` directory (same single-source-of-truth
pattern as `model.py`).
"""
