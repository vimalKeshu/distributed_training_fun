...
def inception_resnet_block(inputs,filter_size,isTraining):
	...
	with tf.variable_scope('Branch_1'):
		...
		branch_1b_conv = tf.layers.conv2d(inputs=branch_1a,filters=filter_size,kernel_initializer=tf.contrib.layers.variance_scaling_initializer(),kernel_regularizer=tf.contrib.layers.l2_regularizer(weight_decay),kernel_size=[3, 3],use_bias=False,padding="same")
	...
	with tf.variable_scope('Branch_2'):
		...
		branch_2b_conv = tf.layers.conv2d(inputs=branch_2a,filters=filter_size,kernel_initializer=tf.contrib.layers.variance_scaling_initializer(),kernel_regularizer=tf.contrib.layers.l2_regularizer(weight_decay),kernel_size=[5, 5],use_bias=False,padding="same")
	...
	merge_branches = tf.concat(axis=3, values=[branch_0, branch_1, branch_2])
	refactored_inputs = tf.layers.conv2d(inputs=inputs,filters=merge_branches.get_shape()[-1],kernel_size=[1, 1],kernel_initializer=tf.contrib.layers.variance_scaling_initializer(),kernel_regularizer=tf.contrib.layers.l2_regularizer(weight_decay),padding="same")
	return tf.nn.relu(merge_branches + refactored_inputs)
...
with tf.variable_scope("inception"):
	...
	for idx in range(4):
		with tf.variable_scope('inception_block'+str(idx+1)):
			net = inception_resnet_block(net,filter_size,isTraining)
