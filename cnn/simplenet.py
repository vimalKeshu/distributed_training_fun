...
with tf.name_scope("pool_layer"+layer_name):
	...
	pool2_dropout = tf.nn.dropout(pool2, dropout_rate)
...
with tf.name_scope("conv_layer"+layer_name):
	...
	conv3 = tf.nn.conv2d(pool2_dropout,filter3,[1,1,1,1],padding="SAME")
...
with tf.name_scope("pool_layer"+layer_name):
	...
	pool3_dropout = tf.nn.dropout(pool3, dropout_rate)
...
with tf.name_scope("conv_layer"+layer_name):
	...
	conv4 = tf.nn.conv2d(pool3_dropout,filter4,[1,1,1,1],padding="SAME")
...
with tf.name_scope("pool_layer"+layer_name):
	...
	pool7_dropout = tf.nn.dropout(pool7, dropout_rate)
...
with tf.name_scope("conv_layer"+layer_name):
	...
	conv8 = tf.nn.conv2d(pool7_dropout,filter8,[1,1,1,1],padding="SAME")
...
with tf.name_scope("pool_layer"+layer_name):
	...
	pool8_dropout = tf.nn.dropout(pool8, dropout_rate)
...
with tf.name_scope("conv_layer"+layer_name):
	...
	conv9 = tf.nn.conv2d(pool8_dropout,filter9,[1,1,1,1],padding="SAME")
...
with tf.name_scope("pool_layer"+layer_name):
	...
	pool9_dropout = tf.nn.dropout(pool9, dropout_rate)
...
with tf.name_scope("pool_layer"+layer_name):
	...
	pool12_dropout = tf.nn.dropout(pool12, dropout_rate)
...
with tf.name_scope("conv_layer"+layer_name):
	...
	conv13 = tf.nn.conv2d(pool12_dropout,filter13,[1,1,1,1],padding="SAME")
with tf.name_scope("pool_layer"+layer_name):
	pool13 = tf.nn.max_pool(conv13, ksize=pooling_size,strides=pooling_stride,padding="SAME",name="pool"+pool_name)
	pool13_dropout = tf.nn.dropout(pool13, dropout_rate)
...
#tf.summary.image("img1",pool13[:,:,:,:3],max_outputs=10)
...
for j in range(epochs):
	...
	for mini_batch in mini_batches:
		...
		session.run(train_op,feed_dict={inputs_data: batch_xs, labels: batch_ys, isTraining: True, dropout_rate: 0.5})
		if step % 10 == 0:
			summary1 = session.run(write_op,feed_dict={inputs_data: batch_xs, labels: batch_ys, isTraining: False, dropout_rate: 1})
			train_writer.add_summary(summary1, step)
			train_writer.flush()

			summary2 = session.run(write_op,feed_dict={inputs_data: test_data[test_mini_batches[0]], labels: test_labels[test_mini_batches[0]], isTraining: False, dropout_rate: 1})
			test_writer.add_summary(summary2, step)
			test_writer.flush()
		step = step + 1
	if (j+1) % 10 == 0:
		for test_mini_batch in test_mini_batches:
			acc = session.run(accuracy,feed_dict={inputs_data: test_data[test_mini_batch], labels: test_labels[test_mini_batch], isTraining: False, dropout_rate: 1})
			accuracy_history.append(acc)
		print('Epoch number {} Test Accuracy: {}'.format(j+1, np.mean(accuracy_history)))
	if (j+1) % 50 == 0:
		saver.save(session,log_dir+"ckpt/model",global_step=50)
